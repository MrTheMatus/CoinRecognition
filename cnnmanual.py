...
wej_arr=[]
train_arr=[]
src1 = r"C:\Users\mrthe\Downloads\archive\coins\data\train"
src2 = r"C:\Users\mrthe\Downloads\archive\coins\data\test"
#LABELS & RESIZE
def resize_img(src1):
    r = 1
...
def resize2_img(src1):
    ...
    for i in os.listdir(src1):
        ...
        if os.path.isfile(current):
            ...
            y2.append(0)
            r = r + 1
for a in os.listdir(src1):
    b=os.path.join(src1,a)
    resize_img(b)
#Reshape features
X = np.reshape(X,(-1,300, 300, 1))
for a in os.listdir(src2):
    b=os.path.join(src2,a)
    resize2_img(b)
X2 = np.reshape(X2,(-1,300, 300, 1))
...
model.summary()
test_loss, test_acc = model.evaluate(X2,  y_test, verbose=1)
print(test_acc)
